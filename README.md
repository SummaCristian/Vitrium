...
`prominent: true` on a tab detaches it into its own floating glass circle (icon-only, like iOS 26's search tab). It's a plain glass button (no sliding pill) that tints its icon when selected, but still a real tab: selecting it clears the main pill and vice versa. Only the last tab can be prominent, and only with at least 3 tabs. With a prominent tab the row switches from centred to space-between, inset from the edges by `--lg-tabbar-inset` (default `1.25rem`). Add `press: true` and it becomes a FAB instead of a tab: clicking it leaves the selection alone and calls the tab's `onPress(el, event)`, e.g. to open an overlay that morphs out of the circle via a View Transition (`tabbar.prominentEl` exposes the element so you can set its `view-transition-name`); it never shows as selected and arrow keys focus it without activating it. `action` is different: a plain glass button beside the bar with no selection state.
...
**Compact layout.** `compact: true` (or `tabbar.setCompact(on, { animate })`) is the iPadOS style, for wide screens only: it replaces the vertical rail with one horizontal bar centred at the top, and leaves the narrow-screen row untouched. `placement` is ignored while it shows. The bar is a single unified pill: a prominent last tab is not detached but stays the bar's last cell and shows only its icon, and every other tab shows only its label. A `press` tab stays a button there (it never selects, it calls `onPress(el)`). Switching in or out animates with the same sequence as `setOrientation()`; `tabbar.compact` reads the option back.
