  ...
  sections() {
    ...
    const playground = createPlayground({
      options: [
        ...
        { key: 'deform', label: 'Stretch on resize', type: 'choice', choices: ['any drag', 'handle', 'off'], default: 'any drag' },
      ...
      code(s) {
        ...
        const rows = [
          ...
          s.deform === 'off' && '  deform: false,',
          s.deform === 'handle' && "  deform: 'handle',",
...
sheet.setSide('start');   // glides across`),
      ...
      section('Content', {},
        ...
        h('ul', { class: 'steps' },
          h('li', {}, h('strong', {}, 'Expand on focus. '), 'Keyboard focus (tabbing in, or tapping a text field) in the content while the sheet is small expands it to the largest detent, so the keyboard does not cover what you are typing into. Turn it off with `expandOnFocus: false`. Focus the search field in the preview while the sheet is at its smallest.'),
      ...
      section('Appearance', {},
        table(['Option', 'What it does'], [
          ...
          ['`deform`', 'Squash and stretch from the resize speed, like the press physics. `true`, `false`, or `"handle"` for drags that start on the grabber or header only. Default: `true`.'],
        ])),
      ...
      section('API', {},
        ...
        table(['Option', 'Type', 'Description'], [
          ...
          ['deform', 'boolean | "handle"', 'Stretch with resize speed. `"handle"` stretches only for a drag that starts on the grabber or header; the content still resizes, without the stretch. Default: `true`.'],
          ['expandOnFocus', 'boolean', 'Expand when keyboard-initiated focus (`:focus-visible`) enters the content. A tap on a card does not. Default: `true`.'],
