...
export default {
  id: 'accent',
  title: 'Accent',
  abstract: 'One colour token that selection, focus and progress all follow.',
  sections() {
    ...
    const picker = h('input', { type: 'color', value: html.style.getPropertyValue('--lg-accent').trim() || currentAccent(), 'aria-label': 'Accent colour' });
    ...
    return [
      section('Overview', {},
        h('p', {}, '--lg-accent is a single custom property. Selection, focus rings, slider fills and progress all read it, so changing it recolours them together.'),
        h('p', {}, 'The toggle keeps its own on-colour token, --lg-toggle-on (iOS green). Pass color: "accent" to a toggle to make it follow the accent, and selectedColor: "accent" to a segmented control to colour its selected label.')),
